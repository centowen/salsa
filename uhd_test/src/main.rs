...
fn measure_switched(usrp: &mut Usrp, sfreq: f64, rfreq: f64, fft_pts : usize, avg_pts : usize, srate: f64, tint: f64, spec : &mut Vec<f64>) {
    let mut spec_sig: Vec<f64> = vec![];
    measure(usrp, sfreq, fft_pts, 0.5*tint, avg_pts, srate, &mut spec_sig);
    let mut spec_ref: Vec<f64> = vec![];
    measure(usrp, rfreq, fft_pts, 0.5*tint, avg_pts, srate, &mut spec_ref);
    for i in 0..avg_pts {
        spec.push(0.5*(spec_sig[i]-spec_ref[i]));
    }
}

fn measure(usrp: &mut Usrp, cfreq: f64, fft_pts: usize, tint: f64, avg_pts: usize, srate: f64, fft_avg : &mut Vec<f64>) {
    ...
    usrp.set_rx_frequency(&TuneRequest::with_frequency(cfreq), 0).unwrap(); // The N210 only has one input channel 0.

    let mut receiver = usrp
        .get_rx_stream(&uhd::StreamArgs::<Complex<i16>>::new("sc16"))
        .unwrap();

    //TODO: Fix so this works with nsamp, check also https://github.com/samcrow/uhd-rust/blob/1ec3f512568b9d0180b1c59107cdbf52506b068c/uhd/src/utils.rs#L100
    //let mut buffer = uhd::alloc_boxed_slice::<Complex<i16>, nsamp >();
    //let mut buffer : Vec<Complex<i16>> = Vec::with_capacity(nsamp as usize);
    //buffer.resize(nsamp as usize, 0.0 as Complex<i16>);
    let mut buffer = uhd::alloc_boxed_slice::<Complex<i16>, {(2.5e6*4.0) as usize} >();
    ...
    for n in 0..nstack {
        let mut fft_buffer: Vec<Complex<f64>> = buffer[n*fft_pts..(n+1)*fft_pts].iter().copied()
            .map(|x| {
...
pub fn main() -> Result<()> {
    ...
    let rfreq: f64 = 1.4265e9;
    let mut spec: Vec<f64> = vec![];
    measure_switched(&mut usrp, sfreq, rfreq, fft_pts, avg_pts, srate, tint, &mut spec);
    ...
    let mut chart = ChartBuilder::on(&root_area)
                    .build_cartesian_2d(0..avg_pts, ymin..ymax).unwrap();
    chart
        .configure_mesh()
        .x_labels(3)
        .y_labels(3)
        .draw()?;
    chart
        .draw_series(LineSeries::new(spec.iter().copied().enumerate(),RED)).unwrap();
